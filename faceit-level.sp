...
void HTTPPlayerDetailsCompleteData(const char[] sData, any iUserID)
{
	...
	if(iClient)
	{
		JSON_Object body = json_decode(sData);
		if (body.HasKey("games") && body.GetObject("games").HasKey("csgo") && body.GetObject("games").GetObject("csgo").HasKey("skill_level")) {
			g_Players[iClient].bLoad = false;
			g_Players[iClient].iSkillLevel = body.GetObject("games").GetObject("csgo").GetInt("skill_level");
		}
	}
}
